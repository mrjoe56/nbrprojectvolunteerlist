...
<div class="messages status no-popup">
    ...
    <h3>{$select_txt}</h3>
    {if !empty($volunteers)}
    <h2>{ts}Add Mailing title{/ts}</h2>

        <div class="crm-form-block nbr-add-followup-block">
            <div class="crm-section mailing_name-section">
                <div class="label">{$form.mailing_name.label}</div>
                <div class="content">{$form.mailing_name.html}</div>
                <div class="clear"></div>
            </div>
            </div>
